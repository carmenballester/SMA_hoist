...
LINE_CODING linecoding = {
  10000000,                            /* baud rate originally 115200*/
  0x00,                                /* stop bits-1*/
  0x00,                                /* parity - none*/
  0x08                                 /* nb. of bits 8*/
};
...
//Sr Antonio
void VCP_send(uint8_t* buf, uint32_t bytes)
{
  VCP_DataTx(buf, bytes);
}
 ...
 * @retval Result of the opeartion: USBD_OK if all operations are OK else VCP_FAIL
 */
#ifndef USB_VCP_STM32F4_REC
#define APP_TX_BUF_SIZE                128

uint8_t APP_Tx_Buffer[APP_TX_BUF_SIZE] __attribute__((section(".ccm")));
...
#ifdef USB_VCP_STM32F4_REC

static uint16_t VCP_DataRx (uint8_t* Buf, uint32_t Len)
{
  uint32_t i;
  for (i = 0; i < Len; i++) {
    *(Target_VCP_Rec_to_PC_BUF_ptr_uint8 + i) = *(Buf + i);
    if (i == CDC_DATA_MAX_PACKET_SIZE)
      return USBD_OK;
  }
